# `components/tables/tabel-1a1-client.tsx`

| | |
|---|---|
| **Area** | Komponen Antarmuka |
| **Ukuran** | 629 baris |
| **Jenis** | Client Component (`"use client"`) |
| **Dipakai oleh** | 0 berkas |

## Maksud berkas

Berkas ini menyediakan 1 fungsi utama: `Tabel1A1Client`.

## Letak berkas

Dari akar repo: `components/tables/tabel-1a1-client.tsx`.

Berkas ini berjalan **di browser pengguna** (`"use client"`). Di dalamnya ada
pengelolaan state dan kejadian klik/ketik. Jangan menaruh kode rahasia
(kata sandi, kunci API) di sini — isinya bisa dibaca pengguna.

## Isi yang bisa dipakai berkas lain

### `Tabel1A1Client`

Jenis: **fungsi**

## Pustaka luar yang dipakai

- `@prisma/client`
- `framer-motion`
- `lucide-react`
- `next/link`
- `next/navigation`
- `react`

## Berkas lain di proyek ini yang dipanggil

- [`components/shared/DosenSelect.tsx`](./components__shared__DosenSelect.md)
- [`lib/actions/lkps.ts`](./lib__actions__lkps.md)
- [`lib/utils/permissions.ts`](./lib__utils__permissions.md)

