# `app/(dashboard)/master/mahasiswa/page.tsx`

| | |
|---|---|
| **Area** | Halaman & API (App Router) |
| **Ukuran** | 190 baris |
| **Jenis** | modul biasa |
| **Alamat URL** | `/master/mahasiswa` |
| **Dipakai oleh** | 0 berkas |

## Maksud berkas

Berkas halaman untuk alamat `/master/mahasiswa`. Tugasnya menyiapkan data di server (dan memeriksa izin), lalu menyerahkan tampilan ke komponen client.

## Letak berkas

Dari akar repo: `app/(dashboard)/master/mahasiswa/page.tsx`.

## Isi yang bisa dipakai berkas lain

### `metadata`

Jenis: **konstanta**

### `MahasiswaPage`

Jenis: **default**

## Alamat yang dilayani

Halaman ini bisa dibuka di `/master/mahasiswa`.

Kalau mau mengubah apa yang tampil di alamat itu, **berkas ini yang pertama dibuka**.

## Pustaka luar yang dipakai

- `@prisma/client`
- `lucide-react`
- `next`
- `next/link`
- `next/navigation`

## Berkas lain di proyek ini yang dipanggil

- [`app/(dashboard)/master/mahasiswa/MahasiswaActions.tsx`](./app__-dashboard-__master__mahasiswa__MahasiswaActions.md)
- [`app/(dashboard)/master/mahasiswa/SearchBar.tsx`](./app__-dashboard-__master__mahasiswa__SearchBar.md)
- [`lib/auth.ts`](./lib__auth.md)
- [`lib/db.ts`](./lib__db.md)
- [`lib/utils/permissions.ts`](./lib__utils__permissions.md)

