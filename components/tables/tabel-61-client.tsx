...
import {
  ...
  Lightbulb, CheckCircle2, Save, Flag, Sparkles
} from "lucide-react";
...
export function Tabel61Client({ initialRows, tahunAkademikId, tabelKode, status, userRole }: Props) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2.5">
          {canEdit && (
            <>
              {visiRows.length === 0 && (
                <button onClick={() => openAdd("VISI")} className="flex items-center gap-1.5 rounded-xl bg-gradient-to-tr from-purple-500 to-indigo-600 px-4 py-2 text-xs font-bold text-white shadow-sm hover:shadow-md transition-all cursor-pointer">
                  <Sparkles className="h-4 w-4" /> Set Visi
                </button>
              )}
              <button onClick={() => openAdd("MISI")} className="flex items-center gap-1.5 rounded-xl bg-gradient-to-tr from-blue-500 to-indigo-600 px-4 py-2 text-xs font-bold text-white shadow-sm hover:shadow-md transition-all cursor-pointer">
      ...
      <div className="flex items-center gap-3 rounded-2xl bg-indigo-50/60 border border-indigo-100/60 px-5 py-4 text-xs font-semibold text-indigo-700">
        ...
        <span>Tabel 6.1 memetakan keselarasan Visi dan Misi antara Perguruan Tinggi (PT), Fakultas (UPPS), dan Program Studi (PS).</span>
      </div>

      {/* SEKSI VISI */}
      <div className="rounded-3xl border border-slate-100/60 bg-white p-7 shadow-sm space-y-5">
        <div className="flex items-center justify-between border-b border-slate-50 pb-4">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-tr from-purple-500 to-indigo-600 text-white shadow-sm">
          ...
          {visiRows.length > 0 && canEdit && visiRows[0] && (
            <button onClick={() => openEdit(visiRows[0]!)} className="flex items-center gap-1 text-xs font-bold text-blue-600 hover:text-blue-700 bg-blue-50 px-3 py-1.5 rounded-xl transition-all cursor-pointer">
        ...
        {visiRows.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {visiRows.map((item) => (
              <div key={item.id} className="rounded-2xl border border-slate-100 bg-slate-50/40 p-5 space-y-2">
                ...
                <p className="text-xs font-semibold text-slate-700 leading-relaxed pt-1">{item.rowData.pt}</p>
              </div>
            ))}
            {visiRows[0] && (
              <>
                ...
                <div className="rounded-2xl border border-slate-100 bg-slate-50/40 p-5 space-y-2">
                  <span className="text-3xs font-extrabold uppercase tracking-wider text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md">Visi PS</span>
      ...
      <div className="rounded-3xl border border-slate-100/60 bg-white p-7 shadow-sm space-y-5">
        <div className="flex items-center justify-between border-b border-slate-50 pb-4">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-tr from-blue-500 to-indigo-600 text-white shadow-sm">
        ...
        {misiRows.length === 0 ? (
          ...
          <div className="space-y-4">
            {misiRows.map((item, idx) => (
              <div key={item.id} className="rounded-2xl border border-slate-100 bg-white p-5 shadow-3xs hover:shadow-md transition-all space-y-3">
                <div className="flex items-center justify-between">
        ...
        {modalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 backdrop-blur-sm p-4 overflow-y-auto">
            <motion.div
              ...
              className="w-full max-w-2xl rounded-3xl bg-white shadow-lg border border-slate-100/50 p-8 my-8"
            >
              <div className="flex items-center gap-4 mb-6">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-tr from-blue-500 to-indigo-600 text-white shadow-sm">
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-5">
                {form.kategori === "MISI" && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-28 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold text-slate-800 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
                  </div>
                )}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <textarea
                      required
                      rows={3}
                      placeholder={`Tuliskan ${form.kategori.toLowerCase()} tingkat Universitas...`}
                      value={form.pt}
                      onChange={(e) => setForm({ ...form, pt: e.target.value })}
                      className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-xs text-slate-800 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      required
                      rows={3}
                      placeholder={`Tuliskan ${form.kategori.toLowerCase()} tingkat Fakultas...`}
                      value={form.upps}
                      onChange={(e) => setForm({ ...form, upps: e.target.value })}
                      className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-xs text-slate-800 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      required
                      rows={3}
                      placeholder={`Tuliskan ${form.kategori.toLowerCase()} tingkat Program Studi...`}
                      value={form.ps}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs text-slate-800 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                  ...
                  <button type="submit" disabled={isLoading} className="flex items-center gap-2 rounded-xl bg-gradient-to-tr from-blue-500 to-indigo-600 px-6 py-2.5 text-xs font-bold text-white shadow-sm hover:shadow-md transition-all disabled:opacity-50">
