...
import {
  ...
  Users, FlaskConical, DollarSign, Edit2, Lightbulb
} from "lucide-react";
...
export function Tabel3A2Client({ initialRows, tahunAkademikId, tabelKode }: Props) {
  ...
  const [form, setForm] = useState({
    namaDtpr: "", judulPenelitian: "", jumlahMahasiswa: "",
    jenisHibah: "L" as "L" | "N" | "I", durasi: "",
    danaTs2: "", danaTs1: "", danaTs: "", linkBukti: "",
  });
  ...
  const openEdit = (item: PenelitianItem) => {
    ...
    setForm({
      ...
      jumlahMahasiswa: String(item.rowData.jumlahMahasiswa || ""),
      jenisHibah: item.rowData.jenisHibah || "L",
  ...
  const handleSave = async () => {
    if (!form.namaDtpr.trim() || !form.judulPenelitian.trim()) {
      triggerToast("Nama DTPR dan Judul harus diisi.", "error");
      return;
    }
    setIsLoading(true);
    try {
      ...
      const updated: PenelitianItem = { id: result.id, rowOrder: result.rowOrder, rowData };
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <Link href="/lkps/bab-3" className="flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-teal-600 transition-colors"><ArrowLeft className="h-4 w-4" /> Kembali ke BAB 3</Link>
        <button onClick={openAdd} className="flex items-center gap-2 rounded-xl bg-gradient-to-tr from-teal-500 to-cyan-600 px-5 py-2.5 text-xs font-bold text-white shadow-soft-sm hover:shadow-soft transition-all"><Plus className="h-4 w-4" /> Tambah Penelitian</button>
      </div>
      ...
      <div className="flex items-center gap-3 rounded-2xl bg-teal-50/60 border border-teal-100/60 px-5 py-4 text-xs font-semibold text-teal-700">
        ...
        <span>Tambah data penelitian DTPR termasuk hibah dan pembiayaan per tahun.</span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-4">
        <div className="rounded-2xl border border-slate-100/50 bg-white p-6 shadow-soft hover:shadow-soft-lg transition-all">
          <div className="space-y-2"><div className="text-xs font-bold uppercase tracking-wider text-slate-400">Jumlah Penelitian</div><p className="text-3xl font-black text-slate-800">{rows.length}</p></div>
        </div>
        <div className="rounded-2xl border border-slate-100/50 bg-white p-6 shadow-soft hover:shadow-soft-lg transition-all">
          <div className="space-y-2"><div className="text-xs font-bold uppercase tracking-wider text-slate-400">Total Dana TS</div><p className="text-3xl font-black text-teal-600">{totalDanaTs.toLocaleString("id-ID")} jt</p></div>
        </div>
        <div className="rounded-2xl border border-slate-100/50 bg-white p-6 shadow-soft hover:shadow-soft-lg transition-all">
          <div className="space-y-2"><div className="text-xs font-bold uppercase tracking-wider text-slate-400">Total Dana TS-1</div><p className="text-3xl font-black text-cyan-600">{totalDanaTs1.toLocaleString("id-ID")} jt</p></div>
        </div>
        <div className="rounded-2xl border border-slate-100/50 bg-white p-6 shadow-soft hover:shadow-soft-lg transition-all">
          <div className="space-y-2"><div className="text-xs font-bold uppercase tracking-wider text-slate-400">Total Dana TS-2</div><p className="text-3xl font-black text-emerald-600">{totalDanaTs2.toLocaleString("id-ID")} jt</p></div>
        </div>
      </div>
        ...
        <div className="bg-gradient-to-r from-teal-500 to-cyan-600 px-7 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/20 backdrop-blur-sm"><FlaskConical className="h-5 w-5 text-white" /></div>
              <div><h3 className="text-sm font-bold text-white">Penelitian DTPR</h3><p className="text-2xs font-semibold text-teal-200">Hibah dan pembiayaan penelitian</p></div>
            </div>
            <span className="rounded-xl bg-white/20 backdrop-blur-sm px-3 py-1 text-2xs font-bold text-white">{rows.length} items</span>
          </div>
        </div>
        {rows.length === 0 ? (
          <div className="p-12 text-center"><div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-slate-100"><FlaskConical className="h-8 w-8 text-slate-400" /></div><p className="text-sm font-semibold text-slate-500">Belum ada data penelitian.</p></div>
        ) : (
            ...
            <table className="w-full text-xs">
              ...
              <tbody>{rows.map((item, idx) => (
                <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-center"><span className={`inline-flex rounded-lg px-2.5 py-1 text-2xs font-bold ${item.rowData.jenisHibah === "I" ? "bg-purple-100 text-purple-700" : item.rowData.jenisHibah === "N" ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-700"}`}>{item.rowData.jenisHibah === "I" ? "Internasional" : item.rowData.jenisHibah === "N" ? "Nasional" : "Lokal"}</span></td>
              ...
              <tfoot><tr className="bg-slate-50 border-t-2 border-slate-200">
                <td colSpan={5} className="px-4 py-3 text-left font-black text-slate-700 uppercase tracking-wider">Total Dana (juta)</td>
                <td className="px-4 py-3 text-center font-black text-emerald-600">{totalDanaTs2 > 0 ? totalDanaTs2 : "-"}</td>
                <td className="px-4 py-3 text-center font-black text-cyan-600">{totalDanaTs1 > 0 ? totalDanaTs1 : "-"}</td>
                <td className="px-4 py-3 text-center font-black text-teal-600">{totalDanaTs > 0 ? totalDanaTs : "-"}</td>
                <td></td>
              </tr></tfoot>
            </table>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 backdrop-blur-sm p-4 overflow-y-auto">
          <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} transition={{ type: "spring", damping: 25, stiffness: 350 }} className="w-full max-w-xl rounded-3xl bg-white shadow-soft-lg border border-slate-100/50 p-8 my-8">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div><h3 className="text-lg font-bold text-slate-800">{editItem ? "Edit Penelitian" : "Tambah Penelitian"}</h3><p className="text-xs text-slate-500 font-semibold mt-0.5">DTPR, Hibah dan Pembiayaan</p></div>
            </div>
            <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-5">
              <div className="rounded-2xl border border-teal-100 bg-teal-50/30 p-5 space-y-4">
                <div className="flex items-center gap-2 text-xs font-bold text-teal-700"><Users className="h-4 w-4" /> Identitas Penelitian</div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-2xs font-bold text-slate-600 mb-1">Nama DTPR (Ketua) <span className="text-red-500">*</span></label><input type="text" placeholder="Nama lengkap" value={form.namaDtpr} onChange={(e) => setForm((p) => ({ ...p, namaDtpr: e.target.value }))} className="w-full rounded-xl border border-teal-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 transition-all focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20 shadow-sm placeholder:text-slate-300" /></div>
                  <div><label className="block text-2xs font-bold text-slate-600 mb-1">Jumlah Mahasiswa</label><input type="number" min="0" placeholder="0" value={form.jumlahMahasiswa} onChange={(e) => setForm((p) => ({ ...p, jumlahMahasiswa: e.target.value }))} className="w-full rounded-xl border border-teal-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 transition-all focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20 shadow-sm placeholder:text-slate-300" /></div>
                </div>
                <div><label className="block text-2xs font-bold text-slate-600 mb-1">Judul Penelitian <span className="text-red-500">*</span></label><input type="text" placeholder="Judul penelitian" value={form.judulPenelitian} onChange={(e) => setForm((p) => ({ ...p, judulPenelitian: e.target.value }))} className="w-full rounded-xl border border-teal-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 transition-all focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20 shadow-sm placeholder:text-slate-300" /></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-2xs font-bold text-slate-600 mb-1">Jenis Hibah</label><div className="flex gap-2">{(["L", "N", "I"] as const).map((v) => (<button key={v} type="button" onClick={() => setForm((p) => ({ ...p, jenisHibah: v }))} className={`flex-1 rounded-xl px-3 py-2.5 text-xs font-bold transition-all ${form.jenisHibah === v ? "bg-gradient-to-tr from-teal-500 to-cyan-600 text-white shadow-soft-sm" : "bg-white border border-slate-200 text-slate-600 hover:border-teal-300"}`}>{v === "L" ? "Lokal" : v === "N" ? "Nasional" : "Internasional"}</button>))}</div></div>
