...
export function Tabel2A3Client({
  initialRows,
  tahunAkademikId,
  tabelKode,
  status,
  ...
  const [jumlahTs2, setJumlahTs2] = useState("");
  const [jumlahTs1, setJumlahTs1] = useState("");
  const [jumlahTs, setJumlahTs] = useState("");
  ...
  const handleOpenEdit = (row: any) => {
    ...
    setJumlahTs2(row.rowData.ts2 !== undefined ? String(row.rowData.ts2) : "");
    setJumlahTs1(row.rowData.ts1 !== undefined ? String(row.rowData.ts1) : "");
    setJumlahTs(row.rowData.ts !== undefined ? String(row.rowData.ts) : "");
    setIsOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const ts2Val = Number(jumlahTs2) || 0;
      const ts1Val = Number(jumlahTs1) || 0;
      const tsVal = Number(jumlahTs) || 0;

      // Simpan 3 nilai ke row yang sama — single upsert, single audit log
      const result = await upsertLkpsRow({
        tabelKode,
        tahunAkademikId,
        rowId: editId,
        rowData: { kategori, ts: tsVal, ts1: ts1Val, ts2: ts2Val },
      });
      ...
      const updatedRow = {
        ...
        rowData: {
          ...
          ts: Number((result.rowData as any)?.ts) || 0,
          ts1: Number((result.rowData as any)?.ts1) || 0,
          ts2: Number((result.rowData as any)?.ts2) || 0,
        },
      };
  ...
  const grandTotal = rows.reduce(
    (acc, r) => acc + (Number(r.rowData.ts2) || 0) + (Number(r.rowData.ts1) || 0) + (Number(r.rowData.ts) || 0),
    0
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <Link href="/lkps/kriteria-2" className="flex items-center gap-2 text-xs font-bold text-slate-800 hover:text-indigo-600 transition-colors">
      ...
      <div className="space-y-3">
        ...
        {rows.length === 0 ? (
          ...
          <>
            {rows.map((row, index) => {
              const rowTotal =
                (Number(row.rowData.ts2) || 0) + (Number(row.rowData.ts1) || 0) + (Number(row.rowData.ts) || 0);
              return (
                <div key={row.id} className="grid grid-cols-12 items-center rounded-3xl bg-white p-4 border border-slate-100/50 shadow-soft hover:shadow-soft-lg transition-all gap-4">
                  ...
                  <div className="col-span-1 flex justify-center gap-1.5">
                    <button onClick={() => handleOpenEdit(row)} disabled={!canEdit} className={`p-1.5 ${canEdit ? "text-slate-400 hover:text-blue-600" : "text-slate-300 cursor-not-allowed"} transition-colors`} title={canEdit ? "Edit" : "Tidak bisa diedit"}>
                      <Edit2 className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={() => setDeleteId(row.id)} disabled={!canEdit} className={`p-1.5 ${canEdit ? "text-slate-400 hover:text-red-600" : "text-slate-300 cursor-not-allowed"} transition-colors`} title={canEdit ? "Hapus" : "Tidak bisa dihapus"}>
        ...
        {isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 overflow-y-auto">
            <motion.div
              ...
              className="relative w-full max-w-lg rounded-3xl bg-white p-5 shadow-soft-lg border border-slate-100 my-4 max-h-[calc(100vh-2rem)] overflow-y-auto"
            >
              {/* Close button */}
              <button
                type="button"
              ...
              <h3 className="text-base font-bold text-slate-800 mb-1 pr-12 flex items-center gap-2">
                <Users className="h-5 w-5 text-indigo-600" />
                {editId ? "Edit Data" : "Tambah Data"}
              </h3>
              <p className="text-2xs font-semibold text-slate-700 mb-4">
                Isi nilai untuk TS-2, TS-1, dan TS.
              </p>

              <form onSubmit={handleSubmit} className="space-y-3">
                {/* Kategori */}
                <div>
                  <label className="block text-2xs font-bold text-slate-800 uppercase mb-1.5">Kondisi Mahasiswa</label>
                  <select
                    required
                    value={kategori}
                    onChange={(e) => setKategori(e.target.value)}
                    className="w-full rounded-xl border border-slate-200 p-2.5 text-xs focus:ring-2 focus:ring-indigo-500/20 outline-none"
                  >
                ...
                {/* 3 input grid: TS-2, TS-1, TS — selalu aktif */}
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block text-2xs font-bold text-slate-800 uppercase mb-1.5">Jumlah TS-2</label>
                    <input
                      type="number"
                      min="0"
                      placeholder="0"
                      value={jumlahTs2}
                      onChange={(e) => setJumlahTs2(e.target.value)}
                      className="w-full rounded-xl border border-slate-200 p-2.5 text-xs focus:ring-2 focus:ring-indigo-500/20 outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-2xs font-bold text-slate-800 uppercase mb-1.5">Jumlah TS-1</label>
                    <input
                      type="number"
                      min="0"
                      placeholder="0"
                      value={jumlahTs1}
                      onChange={(e) => setJumlahTs1(e.target.value)}
                      className="w-full rounded-xl border border-slate-200 p-2.5 text-xs focus:ring-2 focus:ring-indigo-500/20 outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-2xs font-bold text-slate-800 uppercase mb-1.5">Jumlah TS</label>
                    <input
                      type="number"
                      required
                      min="0"
                      placeholder="0"
                      value={jumlahTs}
                      onChange={(e) => setJumlahTs(e.target.value)}
                      className="w-full rounded-xl border border-slate-200 p-2.5 text-xs focus:ring-2 focus:ring-indigo-500/20 outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="flex justify-end gap-2 pt-4 border-t border-slate-50">
                  <button
                    type="button"
                    onClick={() => setIsOpen(false)}
                    className="px-4 py-2 text-xs font-bold text-slate-700 hover:text-slate-600 transition-colors"
                  >
                    Batal
                  </button>
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="rounded-xl bg-indigo-600 px-6 py-2 text-xs font-bold text-white shadow-soft hover:bg-indigo-700 disabled:opacity-50 transition-all"
                  >
