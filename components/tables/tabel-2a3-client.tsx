...
export function Tabel2A3Client({ initialRows, tahunAkademikId, tabelKode }: Tabel2A3ClientProps) {
  ...
  const handleDeleteConfirm = async () => {
    if (!deleteId) return;
    try {
      await deleteLkpsRow(deleteId, `/lkps/bab-2/tabel-2a3`);
              ...
              return (
                <div key={row.id} className="grid grid-cols-12 items-center rounded-3xl bg-white p-4 border border-slate-100/50 shadow-soft hover:shadow-soft-lg transition-all gap-4">
                  ...
                  <div className="col-span-1 flex justify-center gap-1.5">
                    <button onClick={() => handleOpenEdit(row)} className="p-1.5 text-slate-400 hover:text-blue-600 transition-colors"><Edit2 className="h-3.5 w-3.5" /></button>
                    <button onClick={() => setDeleteId(row.id)} className="p-1.5 text-slate-400 hover:text-red-600 transition-colors"><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                </div>
              );
            })}
