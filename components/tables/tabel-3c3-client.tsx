  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
        <div className="rounded-2xl border border-slate-100/50 bg-white p-6 shadow-soft hover:shadow-soft-lg transition-all"><div className="space-y-2"><div className="text-xs font-bold uppercase tracking-wider text-slate-700">Total HKI</div><p className="text-3xl font-black text-slate-800">{rows.length}</p></div></div>
        <div className="rounded-2xl border border-slate-100/50 bg-white p-6 shadow-soft hover:shadow-soft-lg transition-all"><div className="space-y-2"><div className="text-xs font-bold uppercase tracking-wider text-slate-700">Jenis HKI Berbeda</div><p className="text-3xl font-black text-purple-600">{new Set(rows.map((r) => r.rowData.jenisHki)).size}</p></div></div>
        <div className="rounded-2xl border border-slate-100/50 bg-white p-6 shadow-soft hover:shadow-soft-lg transition-all"><div className="space-y-2"><div className="text-xs font-bold uppercase tracking-wider text-slate-700">DTPR Unik</div><p className="text-3xl font-black text-pink-600">{new Set(rows.map((r) => r.rowData.namaDtpr)).size}</p></div></div>
      </div>
      ...
      <div className="rounded-2xl border-2 border-cyan-200/70 bg-white shadow-soft overflow-hidden">
        ...
        {rows.length === 0 ? (
          <div className="p-12 text-center"><div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-slate-100"><Award className="h-8 w-8 text-slate-700" /></div><p className="text-sm font-semibold text-slate-800">Belum ada data HKI.</p></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="border-b border-slate-100 bg-slate-50/50">
                <th className="px-4 py-3 text-left font-bold text-slate-800 uppercase tracking-wider">No</th>
                <th className="px-4 py-3 text-left font-bold text-slate-800 uppercase tracking-wider">Judul HKI</th>
                <th className="px-4 py-3 text-left font-bold text-slate-800 uppercase tracking-wider">Jenis HKI</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 backdrop-blur-sm p-4 overflow-y-auto">
          <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} transition={{ type: "spring", damping: 25, stiffness: 350 }} className="w-full max-w-md rounded-3xl bg-white shadow-soft-lg border border-slate-100/50 p-8 my-8">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div><h3 className="text-lg font-bold text-slate-800">{editItem ? "Edit HKI" : "Tambah Perolehan HKI"}</h3><p className="text-xs text-slate-800 font-semibold mt-0.5">Hak Kekayaan Intelektual</p></div>
            </div>
