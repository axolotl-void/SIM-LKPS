...
import {
  ...
  GraduationCap, Edit2, Lightbulb, AlertTriangle,
} from "lucide-react";
...
export function Tabel3A3Client({ initialRows, tahunAkademikId, tabelKode }: Props) {
  ...
  const openAdd = () => {
    setEditItem(null);
    setForm({ namaDtpr: "", jenisPengembangan: "", linkBukti: "" });
    setModalOpen(true);
  };

  const openEdit = (item: PengembanganItem) => {
    setEditItem(item);
    setForm({
      namaDtpr: item.rowData.namaDtpr,
      jenisPengembangan: item.rowData.jenisPengembangan,
  ...
  const handleSave = async () => {
    if (!form.namaDtpr.trim() || !form.jenisPengembangan.trim()) {
      triggerToast("Nama DTPR dan Jenis Pengembangan harus diisi.", "error");
      return;
    }
    setIsLoading(true);
    try {
      const rowData = {
        ...
        jenisPengembangan: form.jenisPengembangan.trim(),
      ...
      if (isUpdate) setRows((prev) => prev.map((r) => (r.id === editItem.id ? updated : r)));
      else setRows((prev) => [...prev, updated]);
      setModalOpen(false);
      triggerToast(isUpdate ? "Data berhasil diperbarui." : "Data berhasil ditambahkan.", "success");
      router.refresh();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openAdd} className="flex items-center gap-2 rounded-xl bg-gradient-to-tr from-teal-500 to-cyan-600 px-5 py-2.5 text-xs font-bold text-white shadow-soft-sm hover:shadow-soft transition-all">
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50">
                  ...
                  <th className="px-4 py-3 text-left font-bold text-slate-500 uppercase tracking-wider">Link Bukti</th>
                  ...
                  <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {item.rowData.linkBukti ? (
                        <a href={item.rowData.linkBukti} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline font-semibold">Lihat</a>
                      ) : "—"}
                    </td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 overflow-y-auto">
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 16 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 16 }} transition={{ type: "spring", damping: 25, stiffness: 350 }} className="w-full max-w-lg rounded-3xl bg-white shadow-soft-lg border border-slate-100/50 p-8 my-8">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-800">{editItem ? "Edit Pengembangan" : "Tambah Pengembangan"}</h3>
                  <p className="text-xs font-semibold text-slate-400 mt-0.5">Data pengembangan DTPR</p>
                </div>
              </div>
              <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-5">
                <div className="rounded-2xl border border-teal-100 bg-teal-50/30 p-5 space-y-4">
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="cth: Pelatihan, Workshop, Seminar" value={form.jenisPengembangan} onChange={(e) => setForm((p) => ({ ...p, jenisPengembangan: e.target.value }))} className="w-full rounded-xl border border-teal-200 bg-white px-4 py-3 text-sm font-semibold text-slate-800 transition-all focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20 shadow-sm placeholder:text-slate-300" />
                  </div>
                  <div>
                    <label className="block text-2xs font-bold text-slate-600 mb-1">Link Bukti</label>
                    <input type="url" placeholder="https://..." value={form.linkBukti} onChange={(e) => setForm((p) => ({ ...p, linkBukti: e.target.value }))} className="w-full rounded-xl border border-teal-200 bg-white px-4 py-3 text-sm font-semibold text-slate-800 transition-all focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20 shadow-sm placeholder:text-slate-300" />
                  </div>
                </div>
        ...
        {deleteConfirmId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 12 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 12 }} transition={{ type: "spring", damping: 28, stiffness: 380 }} className="w-full max-w-sm rounded-3xl bg-white shadow-soft-lg border border-slate-100/50 p-8">
              ...
              <h3 className="text-base font-bold text-slate-800 text-center">Hapus Data?</h3>
              ...
              <div className="mt-5 rounded-xl bg-red-50/70 border border-red-100 px-4 py-3 text-center">
                <p className="text-xs font-bold text-red-700">&ldquo;{deleteConfirmName || "Item ini"}&rdquo;</p>
              </div>
