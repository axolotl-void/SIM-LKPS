...
import {
  ...
  GraduationCap, BookOpen, Edit2, Lightbulb
} from "lucide-react";
...
interface Props {
  initialRows: PengembanganItem[];
  tahunAkademikId: string;
  tabelKode: string;
}

export function Tabel3A3Client({ initialRows, tahunAkademikId, tabelKode }: Props) {
  const [rows, setRows] = useState(initialRows);
  const router = useRouter();
  const [modalOpen, setModalOpen] = useState(false);
  const [editItem, setEditItem] = useState<PengembanganItem | null>(null);
  const [form, setForm] = useState({ namaDtpr: "", jenisPengembangan: "", linkBukti: "" });
  ...
  const openAdd = () => { setEditItem(null); setForm({ namaDtpr: "", jenisPengembangan: "", linkBukti: "" }); setModalOpen(true); };
  const openEdit = (item: PengembanganItem) => { setEditItem(item); setForm({ namaDtpr: item.rowData.namaDtpr || "", jenisPengembangan: item.rowData.jenisPengembangan || "", linkBukti: item.rowData.linkBukti || "" }); setModalOpen(true); };

  const handleSave = async () => {
    if (!form.namaDtpr.trim() || !form.jenisPengembangan.trim()) { triggerToast("Nama DTPR dan Jenis Pengembangan harus diisi.", "error"); return; }
    setIsLoading(true);
    try {
      const rowData = { tahun: "TS", namaDtpr: form.namaDtpr.trim(), jenisPengembangan: form.jenisPengembangan.trim(), linkBukti: form.linkBukti.trim() };
      ...
      const updated: PengembanganItem = { id: result.id, rowOrder: result.rowOrder, rowData };
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <Link href="/lkps/bab-3" className="flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-cyan-600 transition-colors"><ArrowLeft className="h-4 w-4" /> Kembali ke BAB 3</Link>
        <button onClick={openAdd} className="flex items-center gap-2 rounded-xl bg-gradient-to-tr from-cyan-500 to-blue-600 px-5 py-2.5 text-xs font-bold text-white shadow-soft-sm hover:shadow-soft transition-all"><Plus className="h-4 w-4" /> Tambah Pengembangan</button>
      </div>
      ...
      <div className="flex items-center gap-3 rounded-2xl bg-cyan-50/60 border border-cyan-100/60 px-5 py-4 text-xs font-semibold text-cyan-700">
        ...
        <span>Catat pengembangan DTPR di bidang penelitian. Pengisian tidak berulang — dosen yang dikirim tugas belajar tidak dihitung lagi.</span>
      </div>

      <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
        <div className="rounded-2xl border border-slate-100/50 bg-white p-6 shadow-soft hover:shadow-soft-lg transition-all"><div className="space-y-2"><div className="text-xs font-bold uppercase tracking-wider text-slate-400">Total DTPR</div><p className="text-3xl font-black text-slate-800">{rows.length}</p></div></div>
        <div className="rounded-2xl border border-slate-100/50 bg-white p-6 shadow-soft hover:shadow-soft-lg transition-all"><div className="space-y-2"><div className="text-xs font-bold uppercase tracking-wider text-slate-400">Jenis Pengembangan</div><p className="text-3xl font-black text-cyan-600">{new Set(rows.map((r) => r.rowData.jenisPengembangan)).size}</p></div></div>
        <div className="rounded-2xl border border-slate-100/50 bg-white p-6 shadow-soft hover:shadow-soft-lg transition-all"><div className="space-y-2"><div className="text-xs font-bold uppercase tracking-wider text-slate-400">Dosen Unik</div><p className="text-3xl font-black text-blue-600">{new Set(rows.map((r) => r.rowData.namaDtpr)).size}</p></div></div>
      </div>
        ...
        <div className="bg-gradient-to-r from-cyan-500 to-blue-600 px-7 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/20 backdrop-blur-sm"><GraduationCap className="h-5 w-5 text-white" /></div>
              <div><h3 className="text-sm font-bold text-white">Pengembangan DTPR</h3><p className="text-2xs font-semibold text-cyan-200">Di Bidang Penelitian</p></div>
            </div>
            <span className="rounded-xl bg-white/20 backdrop-blur-sm px-3 py-1 text-2xs font-bold text-white">{rows.length} items</span>
          </div>
        </div>
        {rows.length === 0 ? (
          <div className="p-12 text-center"><div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-slate-100"><GraduationCap className="h-8 w-8 text-slate-400" /></div><p className="text-sm font-semibold text-slate-500">Belum ada data pengembangan.</p></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="border-b border-slate-100 bg-slate-50/50">
                ...
                <th className="px-4 py-3 text-left font-bold text-slate-500 uppercase tracking-wider">Jenis Pengembangan</th>
              ...
              <tbody>{rows.map((item, idx) => (
                <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-slate-600">{item.rowData.jenisPengembangan}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 backdrop-blur-sm p-4 overflow-y-auto">
          <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} transition={{ type: "spring", damping: 25, stiffness: 350 }} className="w-full max-w-md rounded-3xl bg-white shadow-soft-lg border border-slate-100/50 p-8 my-8">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div><h3 className="text-lg font-bold text-slate-800">{editItem ? "Edit Pengembangan" : "Tambah Pengembangan"}</h3><p className="text-xs text-slate-500 font-semibold mt-0.5">DTPR di Bidang Penelitian</p></div>
            </div>
            <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-5">
              <div className="rounded-2xl border border-cyan-100 bg-cyan-50/30 p-5 space-y-4">
                <div className="flex items-center gap-2 text-xs font-bold text-cyan-700"><GraduationCap className="h-4 w-4" /> Data DTPR</div>
                ...
                <div><label className="block text-2xs font-bold text-slate-600 mb-1">Jenis Pengembangan <span className="text-red-500">*</span></label><input type="text" placeholder="cth: Tugas Belajar, Seminar, Workshop" value={form.jenisPengembangan} onChange={(e) => setForm((p) => ({ ...p, jenisPengembangan: e.target.value }))} className="w-full rounded-xl border border-cyan-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 transition-all focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 shadow-sm placeholder:text-slate-300" /></div>
                <div><label className="block text-2xs font-bold text-slate-600 mb-1">Link Bukti</label><input type="url" placeholder="https://..." value={form.linkBukti} onChange={(e) => setForm((p) => ({ ...p, linkBukti: e.target.value }))} className="w-full rounded-xl border border-cyan-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 transition-all focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 shadow-sm placeholder:text-slate-300" /></div>
              </div>
