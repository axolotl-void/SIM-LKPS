  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <Link href="/lkps/kriteria-3" className="flex items-center gap-2 text-xs font-bold text-slate-800 hover:text-indigo-600 transition-colors"><ArrowLeft className="h-4 w-4" /> Kembali ke Kriteria 3</Link>
        <div className="flex items-center gap-2.5">
          <button onClick={openAdd} disabled={!canEdit} className={`flex items-center gap-2 rounded-xl px-5 py-2.5 text-xs font-bold shadow-soft-sm hover:shadow-soft transition-all ${canEdit ? "bg-gradient-to-tr from-cyan-500 to-blue-600 text-white" : "bg-slate-100 text-slate-400 cursor-not-allowed"}`}><Plus className="h-4 w-4" /> Tambah Publikasi</button>
        </div>
      </div>
      ...
      <div className="rounded-2xl border-2 border-cyan-200/70 bg-white shadow-soft overflow-hidden">
        ...
        {rows.length === 0 ? (
          <div className="p-12 text-center"><div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-slate-100"><BookOpen className="h-8 w-8 text-slate-700" /></div><p className="text-sm font-semibold text-slate-800">Belum ada data publikasi.</p></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="border-b border-slate-100 bg-slate-50/50">
                ...
                <th className="px-4 py-3 text-left font-bold text-slate-800 uppercase tracking-wider">Judul Publikasi</th>
                <th className="px-4 py-3 text-center font-bold text-slate-800 uppercase tracking-wider">Jenis</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 backdrop-blur-sm p-4 overflow-y-auto">
          <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} transition={{ type: "spring", damping: 25, stiffness: 350 }} className="w-full max-w-lg rounded-3xl bg-white shadow-soft-lg border border-slate-100/50 p-8 my-8">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div><h3 className="text-lg font-bold text-slate-800">{editItem ? "Edit Publikasi" : "Tambah Publikasi Penelitian"}</h3><p className="text-xs text-slate-800 font-semibold mt-0.5">Publikasi dan jenisnya</p></div>
            </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {JENIS_OPTIONS.map((opt) => (
                      <button key={opt.value} type="button" onClick={() => setForm((p) => ({ ...p, jenisPublikasi: opt.value }))} className={`rounded-xl px-3 py-2.5 text-2xs font-bold transition-all ${form.jenisPublikasi === opt.value ? "bg-gradient-to-tr from-cyan-500 to-blue-600 text-white shadow-soft-sm" : "bg-white border border-slate-200 text-slate-600 hover:border-indigo-300"}`}>{opt.value}</button>
                    ))}
                  </div>
                  <p className="text-2xs text-slate-700 mt-1">IB=Internasional Bereputasi, S1-S4=Jurnal Sinta, T=Tidak Terakreditasi</p>
                </div>
