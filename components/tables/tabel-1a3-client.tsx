...
export function Tabel1A3Client({ initialRows, tahunAkademikId, tabelKode }: Tabel1A3ClientProps) {
  ...
  const [nominal, setNominal] = useState(""); // Formatted string with dots
  ...
  // Predefined options for usage of funds
  const penggunaanPilihan = [
    "Biaya Operasional Pendidikan",
    "Biaya Penelitian Dosen",
    "Biaya Pengabdian kepada Masyarakat (PkM) Dosen",
    "Biaya Investasi SDM (Pelatihan, S3, dll)",
    "Biaya Investasi Sarana & Prasarana",
    "Biaya Investasi Teknologi Informasi (TI)",
    "Biaya Operasional Non-Akademik / Lain-lain"
  ];
  ...
  const handleOpenEdit = (row: any) => {
    setEditId(row.id);
    setJenisPenggunaan(row.rowData.jenisPenggunaan || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editId) {
        ...
        triggerToast("Perubahan penggunaan dana berhasil disimpan", "success");
      } else {
        setRows([...rows, updatedRow]);
        triggerToast("Penggunaan dana baru berhasil ditambahkan", "success");
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setDeleteId(null);
      triggerToast("Data penggunaan dana berhasil dihapus", "success");
      router.refresh();
    } catch (err) {
      console.error(err);
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="grid grid-cols-12 px-6 py-2.5 text-2xs font-bold uppercase tracking-wider text-slate-400 select-none">
          ...
          <div className="col-span-3">Jenis Penggunaan Dana</div>
        ...
        {rows.length === 0 ? (
          <div className="rounded-3xl bg-white border border-slate-100 p-10 text-center text-xs font-semibold text-slate-400 shadow-soft">
            Belum ada data penggunaan dana. Silakan tambah data baru.
          </div>
        ) : (
          <>
            {rows.map((row, index) => (
            ...
            <div className="grid grid-cols-12 items-center rounded-3xl bg-slate-50/50 p-5 border border-slate-200/60 shadow-3xs gap-4 font-bold">
              <div className="col-span-4 text-xs text-slate-800 text-left font-extrabold pl-2">
                Jumlah Total Alokasi Penggunaan
              </div>
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-800 tracking-tight">
                    {editId ? "Ubah Penggunaan Dana" : "Tambah Penggunaan Dana"}
                  </h3>
                  <p className="text-xs text-slate-500 font-semibold mt-0.5">
                    Lengkapi nominal alokasi penggunaan dana untuk Tahun Sekarang (TS)
                  </p>
                </div>
              </div>

              <form onSubmit={handleSubmit} className="space-y-6">
                {/* Card Section: Rincian Penggunaan */}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="md:col-span-2">
                      <label className="block text-2xs font-bold text-slate-700 mb-1.5">
                        Jenis Penggunaan Dana <span className="text-red-500">*</span>
                      </label>
                      <div className="relative">
                        ...
                        >
                          <option value="" disabled>Pilih Jenis Penggunaan Dana</option>
                          {penggunaanPilihan.map((item) => (
