...
const Tabel4C3Client = dynamic(() =>
  import("@/components/tables/tabel-4c3-client").then((m) => m.Tabel4C3Client)
);
import { ValidationHistory } from "@/components/tables/validation-history";
import { ErrorBoundary } from "@/components/shared/error-boundary";
import { Award, Calendar, BookOpen, CheckCircle2, Clock, AlertCircle, XCircle } from "lucide-react";
import { Role } from "@prisma/client";
import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Tabel 4.C.3 — Perolehan HKI PkM",
};
...
export default async function Tabel4C3Page() {
  ...
  const def = await db.tabelDefinition.findUnique({ where: { kode: "4.C.3" } });
  if (!def) {
    return (
      <div className="rounded-2xl bg-white p-6 shadow-soft text-center text-xs font-semibold text-slate-500">
        Definisi Tabel 4.C.3 tidak ditemukan. Pastikan seed data telah dijalankan.
      </div>
    );
  }
  ...
  const rows = rawRows.map((r) => {
    ...
    return {
      ...
      rowData: {
        ...
        ts2: d.ts2 === true || d.ts2 === "true" || d.ts2 === 1,
        ts1: d.ts1 === true || d.ts1 === "true" || d.ts1 === 1,
        ts: d.ts === true || d.ts === "true" || d.ts === 1,
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex flex-col gap-2.5 md:max-w-2xl">
          <div>
            ...
            <p className="mt-1 text-xs font-semibold text-slate-500">
              Perolehan Hak Kekayaan Intelektual dari hasil pengabdian kepada masyarakat
            </p>
          </div>
      ...
      <ErrorBoundary>
        <Tabel4C3Client initialRows={rows} tahunAkademikId={activeTa.id} tabelKode={def.kode} status={status} userRole={session.user.role as Role} />
      </ErrorBoundary>
