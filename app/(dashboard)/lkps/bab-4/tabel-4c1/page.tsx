...
const Tabel4C1Client = dynamic(() =>
  import("@/components/tables/tabel-4c1-client").then((m) => m.Tabel4C1Client)
);
import { ValidationHistory } from "@/components/tables/validation-history";
import { ErrorBoundary } from "@/components/shared/error-boundary";
import { Handshake, Calendar, BookOpen, CheckCircle2, Clock, AlertCircle, XCircle } from "lucide-react";
import { Role } from "@prisma/client";
import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Tabel 4.C.1 — Kerja Sama PkM",
};
...
export default async function Tabel4C1Page() {
  ...
  const def = await db.tabelDefinition.findUnique({ where: { kode: "4.C.1" } });
  if (!def) {
    return (
      <div className="rounded-2xl bg-white p-6 shadow-soft text-center text-xs font-semibold text-slate-500">
        Definisi Tabel 4.C.1 tidak ditemukan. Pastikan seed data telah dijalankan.
      </div>
    );
  }
  ...
  const rows = rawRows.map((r) => {
    ...
    return {
      ...
      rowData: {
        ...
        judulKerjasama: String(d.judulKerjasama ?? ""),
        mitraKerja: String(d.mitraKerja ?? ""),
        sumber: (["L", "N", "I"].includes(String(d.sumber)) ? d.sumber : "L") as "L" | "N" | "I",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-white p-4 shadow-soft-sm border border-slate-100/60">
        <div className="absolute right-0 top-0 bottom-0 w-1/3 bg-gradient-to-l from-teal-50/50 via-cyan-50/20 to-transparent pointer-events-none rounded-r-3xl" />
        <div className="absolute right-12 top-1/2 -translate-y-1/2 hidden md:block pointer-events-none">
          <div className="flex h-20 w-16 rotate-12 items-center justify-center rounded-2xl bg-white shadow-soft-lg border border-slate-100/40 text-teal-500">
        ...
        <div className="relative z-10 flex flex-col gap-2.5 md:max-w-2xl">
          <div>
            <div className="flex items-center gap-3">
              <span className="text-3xs font-black uppercase tracking-wider text-teal-600 bg-teal-50/80 px-2.5 py-1 rounded-lg">
                Tabel {def.kode}
              </span>
            ...
            <p className="mt-1 text-xs font-semibold text-slate-500">
              Kerja sama pengabdian kepada masyarakat dengan mitra
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-2 rounded-lg bg-slate-50/70 p-2">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-tr from-teal-500/10 to-cyan-500/10 text-teal-600 shadow-soft-2xs">
      ...
      <ErrorBoundary>
        <Tabel4C1Client initialRows={rows} tahunAkademikId={activeTa.id} tabelKode={def.kode} status={status} userRole={session.user.role as Role} />
      </ErrorBoundary>
