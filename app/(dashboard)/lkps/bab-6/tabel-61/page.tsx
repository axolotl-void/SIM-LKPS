...
const Tabel61Client = dynamic(() =>
  import("@/components/tables/tabel-61-client").then((m) => m.Tabel61Client)
);
import { ValidationHistory } from "@/components/tables/validation-history";
import { ErrorBoundary } from "@/components/shared/error-boundary";
import { BookOpen, Calendar, Eye, CheckCircle2, Clock, AlertCircle, XCircle } from "lucide-react";
import { Role } from "@prisma/client";
import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Tabel 6.1 — Visi Misi Tujuan",
};
...
export default async function Tabel61Page() {
  ...
  const def = await db.tabelDefinition.findUnique({ where: { kode: "6.1" } });
  if (!def) {
    return (
      <div className="rounded-2xl bg-white p-6 shadow-soft text-center text-xs font-semibold text-slate-500">
        Definisi Tabel 6.1 tidak ditemukan. Pastikan database seed telah dijalankan.
      </div>
    );
  }
  ...
  const initialRows = (lkps?.rows || []).map((r) => {
    ...
    return {
      ...
      rowOrder: r.rowOrder,
      rowData: {
        kategori: (d.kategori as "VISI" | "MISI") || "MISI",
        nomor: d.nomor as number | undefined,
        pt: String(d.pt || ""),
        upps: String(d.upps || ""),
        ps: String(d.ps || ""),
        linkBukti: d.linkBukti as string | undefined,
      },
    };
  });
