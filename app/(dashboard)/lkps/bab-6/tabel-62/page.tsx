...
const Tabel62Client = dynamic(() =>
  import("@/components/tables/tabel-62-client").then((m) => m.Tabel62Client)
);
...
export const metadata: Metadata = {
  title: "Tabel 6.2 — Strategi Pencapaian",
};
...
export default async function Tabel62Page() {
  ...
  const def = await db.tabelDefinition.findUnique({ where: { kode: "6.2" } });
  if (!def) {
    return (
      <div className="rounded-2xl bg-white p-6 shadow-soft text-center text-xs font-semibold text-slate-500">
        Definisi Tabel 6.2 tidak ditemukan. Pastikan database seed telah dijalankan.
      </div>
    );
  }
  ...
  const initialRows = (lkps?.rows || []).map((r) => {
    ...
    return {
      ...
      rowOrder: r.rowOrder,
      rowData: {
        nomor: Number(d.nomor || 1),
        strategi: String(d.strategi || ""),
        sasaran: String(d.sasaran || ""),
        indikator: String(d.indikator || ""),
