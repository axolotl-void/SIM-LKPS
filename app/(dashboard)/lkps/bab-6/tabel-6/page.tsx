...
import { Tabel6Client } from "@/components/tables/tabel-6-client";
...
export const metadata: Metadata = {
  title: "Tabel 6 — Kesesuaian Visi dan Misi",
};
...
export default async function Tabel6Page() {
  ...
  const def = await db.tabelDefinition.findUnique({ where: { kode: "6" } });
  if (!def) {
    return (
      <div className="rounded-2xl bg-white p-6 shadow-soft text-center text-xs font-semibold text-slate-500">
        Definisi Tabel 6 tidak ditemukan. Pastikan database seed telah dijalankan.
      </div>
    );
  }
