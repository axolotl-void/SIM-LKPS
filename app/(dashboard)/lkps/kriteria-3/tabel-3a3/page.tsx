...
const Tabel3A3Client = dynamic(() =>
  import("@/components/tables/tabel-3a3-client").then((m) => m.Tabel3A3Client)
);
...
export const metadata: Metadata = {
  title: "Tabel 3.A.3 — Pengembangan DTPR di Bidang Penelitian",
};
...
export default async function Tabel3A3Page() {
  ...
  const [activeTa, def, dosens] = await Promise.all([
    ...
    await db.tabelDefinition.findUnique({ where: { kode: "3.A.3" } }),
  ...
  if (!def) {
    return (
      <div className="rounded-2xl bg-white p-6 shadow-soft text-center text-xs font-semibold text-slate-500">
        Definisi Tabel 3.A.3 tidak ditemukan. Pastikan seed data telah dijalankan.
      </div>
    );
  }
  ...
  const rows = rawRows.map((r) => {
    ...
    return {
      ...
      rowData: {
        ...
        jenisPengembangan: String(d.jenisPengembangan ?? ""),
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex flex-col gap-2.5 md:max-w-2xl">
          <div>
            ...
            <p className="mt-1 text-xs font-semibold text-slate-500">
              Pengembangan DTPR di bidang penelitian
            </p>
          </div>
      ...
      <ErrorBoundary>
        <Tabel3A3Client initialRows={rows} tahunAkademikId={activeTa.id} tabelKode={def.kode} status={status}
