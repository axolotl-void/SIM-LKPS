...
export default async function Page() {
  ...
  const def = await db.tabelDefinition.findFirst({ where: { kode: "3.C.3" } });
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-3xl bg-white p-7 shadow-soft border border-slate-100/50">
        ...
        <div className="relative z-10 flex flex-col gap-5 md:max-w-xl">
          <div>
            <span className="text-3xs font-black uppercase tracking-wider text-purple-600 bg-purple-50/80 px-2.5 py-1 rounded-lg">Tabel {def.kode}</span>
            <h2 className="mt-3.5 text-lg font-bold text-slate-800 tracking-tight">{def.nama}</h2>
            <p className="mt-1 text-xs font-semibold text-slate-500">Perolehan HKI dari Penelitian DTPR</p>
          </div>
        </div>
      </div>
      <ErrorBoundary>
