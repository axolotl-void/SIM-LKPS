...
const Tabel3C3Client = dynamic(() =>
  import("@/components/tables/tabel-3c3-client").then((m) => m.Tabel3C3Client)
);
import { ValidationHistory } from "@/components/tables/validation-history";
import { ErrorBoundary } from "@/components/shared/error-boundary";
import { Award, CheckCircle2, Clock, AlertCircle, XCircle, Calendar, BookOpen } from "lucide-react";
...
export default async function Tabel3C3Page() {
  ...
  const [activeTa, def, dosens] = await Promise.all([
    ...
    await db.tabelDefinition.findUnique({ where: { kode: "3.C.3" } }),
  ...
  return (
    <div className="space-y-6">
      ...
      <ErrorBoundary>
        <Tabel3C3Client initialRows={rows} tahunAkademikId={activeTa.id} tabelKode={def.kode} status={status}
