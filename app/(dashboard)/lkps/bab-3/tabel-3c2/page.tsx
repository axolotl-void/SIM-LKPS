...
const Tabel3C2Client = dynamic(() =>
  import("@/components/tables/tabel-3c2-client").then((m) => m.Tabel3C2Client)
);
...
export default async function Tabel3C2Page() {
  ...
  const [activeTa, def, dosens] = await Promise.all([
    ...
    await db.tabelDefinition.findUnique({ where: { kode: "3.C.2" } }),
  ...
  return (
    <div className="space-y-6">
      ...
      <ErrorBoundary>
        <Tabel3C2Client initialRows={rows} tahunAkademikId={activeTa.id} tabelKode={def.kode} status={status}
