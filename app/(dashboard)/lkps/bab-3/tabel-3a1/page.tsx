...
const Tabel3A1Client = dynamic(() =>
  import("@/components/tables/tabel-3a1-client").then((m) => m.Tabel3A1Client)
);
import { ValidationHistory } from "@/components/tables/validation-history";
import { ErrorBoundary } from "@/components/shared/error-boundary";
import { Building2, CheckCircle2, Clock, AlertCircle, XCircle, Calendar, BookOpen } from "lucide-react";
...
export default async function Tabel3A1Page() {
  ...
  const def = await db.tabelDefinition.findUnique({ where: { kode: "3.A.1" } });
  if (!def) {
    return (
      <div className="rounded-2xl bg-white p-6 shadow-soft text-center text-xs font-semibold text-slate-500">
        Definisi Tabel 3.A.1 tidak ditemukan. Pastikan seed data telah dijalankan.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex flex-col gap-2.5 md:max-w-2xl">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-2 rounded-lg bg-slate-50/70 p-2">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-tr from-violet-500/10 to-teal-500/10 text-violet-600 shadow-soft-2xs">
      ...
      <ErrorBoundary>
        <Tabel3A1Client initialRows={rows} tahunAkademikId={activeTa.id} tabelKode={def.kode} status={status} userRole={session.user.role as Role} />
      </ErrorBoundary>
      {history.length > 0 && (
