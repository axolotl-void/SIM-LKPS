...
import { FlaskConical, Calendar, BookOpen, FileText } from "lucide-react";
import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Tabel 3.A.2 — Penelitian DTPR, Hibah, dan Pembiayaan",
};

export default async function Tabel3A2Page() {
  ...
  if (!def) {
    return (
      <div className="rounded-2xl bg-white p-6 shadow-soft text-center text-xs font-semibold text-slate-500">
        Definisi Tabel 3.A.2 tidak ditemukan. Pastikan seed data telah dijalankan.
      </div>
    );
  }
  ...
  // Cast Prisma JsonValue → PenelitianItem agar client component dapat typed data
  const rows = rawRows.map((r) => {
    ...
    return {
      ...
      rowData: {
        ...
        judulPenelitian: String(d.judulPenelitian ?? ""),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-white p-7 shadow-soft border border-slate-100/50">
        ...
        <div className="absolute right-12 top-1/2 -translate-y-1/2 hidden md:block pointer-events-none">
          <div className="flex h-20 w-16 rotate-12 items-center justify-center rounded-2xl bg-white shadow-soft-lg border border-slate-100/40 text-teal-500">
            <FlaskConical className="h-10 w-10 text-teal-400" />
          </div>
        </div>
        ...
        <div className="relative z-10 flex flex-col gap-5 md:max-w-xl">
          <div>
            ...
            <p className="mt-1 text-xs font-semibold text-slate-500">
              Penelitian DTPR, hibah, dan pembiayaan penelitian
            </p>
          </div>
      ...
      <ErrorBoundary>
        <Tabel3A2Client initialRows={rows} tahunAkademikId={activeTa.id} tabelKode={def.kode} />
      </ErrorBoundary>
    </div>
  );
}
