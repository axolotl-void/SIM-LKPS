...
import { Tabel3C1Client } from "@/components/tables/tabel-3c1-client";
import { ValidationHistory } from "@/components/tables/validation-history";
import { ErrorBoundary } from "@/components/shared/error-boundary";
import { Handshake, Calendar, BookOpen, FileText, CheckCircle2, Clock, AlertCircle, XCircle } from "lucide-react";
import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Tabel 3.C.1 — Kerja Sama Penelitian",
};
...
export default async function Tabel3C1Page() {
  ...
  const def = await db.tabelDefinition.findUnique({ where: { kode: "3.C.1" } });
  if (!def) {
    return (
      <div className="rounded-2xl bg-white p-6 shadow-soft text-center text-xs font-semibold text-slate-500">
        Definisi Tabel 3.C.1 tidak ditemukan. Pastikan seed data telah dijalankan.
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-white p-7 shadow-soft border border-slate-100/50">
        ...
        <div className="absolute right-12 top-1/2 -translate-y-1/2 hidden md:block pointer-events-none">
          <div className="flex h-20 w-16 rotate-12 items-center justify-center rounded-2xl bg-white shadow-soft-lg border border-slate-100/40 text-blue-500">
        ...
        <div className="relative z-10 flex flex-col gap-5 md:max-w-xl">
          <div>
            ...
            <p className="mt-1 text-xs font-semibold text-slate-500">
              Kerja sama penelitian dengan mitra
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-3 rounded-2xl bg-white p-3.5 shadow-[0_8px_30px_rgba(0,0,0,0.03)] border border-slate-100/50">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-tr from-blue-500/10 to-indigo-500/10 text-blue-600 shadow-soft-2xs">
      ...
      <ErrorBoundary>
        <Tabel3C1Client initialRows={rows} tahunAkademikId={activeTa.id} tabelKode={def.kode} status={status} userRole={session.user.role} />
      </ErrorBoundary>
