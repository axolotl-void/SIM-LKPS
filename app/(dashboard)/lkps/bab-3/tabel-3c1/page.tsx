...
const Tabel3C1Client = dynamic(() =>
  import("@/components/tables/tabel-3c1-client").then((m) => m.Tabel3C1Client)
);
import { ValidationHistory } from "@/components/tables/validation-history";
import { ErrorBoundary } from "@/components/shared/error-boundary";
import { Handshake, CheckCircle2, Clock, AlertCircle, XCircle, Calendar, BookOpen } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-white p-4 shadow-soft-sm border border-slate-100/60">
        <div className="absolute right-0 top-0 bottom-0 w-1/3 bg-gradient-to-l from-violet-50/50 via-indigo-50/20 to-transparent pointer-events-none rounded-r-3xl" />
        <div className="absolute right-12 top-1/2 -translate-y-1/2 hidden md:block pointer-events-none">
          <div className="flex h-20 w-16 rotate-12 items-center justify-center rounded-2xl bg-white shadow-soft-lg border border-slate-100/40 text-violet-500">
            <Handshake className="h-10 w-10 text-violet-400" />
          </div>
        </div>
      ...
      <ErrorBoundary>
        <Tabel3C1Client initialRows={rows} tahunAkademikId={activeTa.id} tabelKode={def.kode} status={status} userRole={session.user.role as Role} />
      </ErrorBoundary>
