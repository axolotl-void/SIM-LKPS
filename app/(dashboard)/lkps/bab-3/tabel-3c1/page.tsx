...
export default async function Page() {
  ...
  const def = await db.tabelDefinition.findFirst({ where: { kode: "3.C.1" } });
      ...
      <div className="relative overflow-hidden rounded-3xl bg-white p-7 shadow-soft border border-slate-100/50">
        ...
        <div className="relative z-10 flex flex-col gap-5 md:max-w-xl">
          <div>
            ...
            <p className="mt-1 text-xs font-semibold text-slate-500">Kerjasama Penelitian dengan Mitra Kerja</p>
          </div>
        </div>
      </div>
      <ErrorBoundary>
