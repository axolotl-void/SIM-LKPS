...
const Tabel2B2Client = dynamic(() =>
  import("@/components/tables/tabel-2b2-client").then((m) => m.Tabel2B2Client)
);
...
export default async function Tabel2B2Page() {
  ...
  const [activeTa, def] = await Promise.all([
    ...
    await db.tabelDefinition.findUnique({ where: { kode: "2.B.2" } }),
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <ErrorBoundary>
        <Tabel2B2Client initialRows={lkpsTs?.rows || []} tahunAkademikId={activeTa.id} tabelKode={def.kode} status={status} userRole={session.user.role as Role} />
      </ErrorBoundary>
      {history.length > 0 && (
