...
const Tabel2CClient = dynamic(() =>
  import("@/components/tables/tabel-2c-client").then((m) => m.Tabel2CClient)
);
...
export default async function Tabel2CPage() {
  ...
  const [activeTa, def] = await Promise.all([
    ...
    await db.tabelDefinition.findUnique({ where: { kode: "2.C" } }),
  ]);
  ...
  const buildRows = (rows: { rowData: unknown }[], _tahun: string) => {
    ...
    for (const t of DEFAULT_TYPES) {
      const match = rows.find((r) => (r.rowData as Record<string, unknown> | null)?.key === t.key);
      result[t.key] = match ? (match.rowData as Record<string, unknown>) : null;
    }
    return result;
  };
