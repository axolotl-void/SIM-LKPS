...
const Tabel2A2Client = dynamic(() =>
  import("@/components/tables/tabel-2a2-client").then((m) => m.Tabel2A2Client)
);
...
export default async function Tabel2A2Page() {
  ...
  return (
    <div className="space-y-6">
      ...
      <ErrorBoundary>
        <Tabel2A2Client initialRows={initialRows} tahunAkademikId={activeTa.id} tabelKode={def.kode} status={status} userRole={session.user.role as Role} />
      </ErrorBoundary>
      {history.length > 0 && (
