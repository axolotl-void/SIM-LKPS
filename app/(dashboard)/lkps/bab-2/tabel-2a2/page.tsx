...
import { Tabel2A2Client } from "@/components/tables/tabel-2a2-client";
...
export default async function Tabel2A2Page() {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl bg-white p-7 shadow-soft border border-slate-100/50">
        ...
        <p className="mt-1 text-xs font-semibold text-slate-400">Data sebaran asal daerah mahasiswa baru {activeTa.tahun} ({activeTa.semester})</p>
      </div>
      <ErrorBoundary>
        <Tabel2A2Client initialRows={initialRows} tahunAkademikId={activeTa.id} tabelKode={def.kode} />
      </ErrorBoundary>
    </div>
  );
}
