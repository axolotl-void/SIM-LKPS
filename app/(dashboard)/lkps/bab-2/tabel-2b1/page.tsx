...
const Tabel2B1Client = dynamic(() =>
  import("@/components/tables/tabel-2b1-client").then((m) => m.Tabel2B1Client)
);
...
export default async function Tabel2B1Page() {
  ...
  return (
    <div className="space-y-6">
      ...
      <ErrorBoundary>
        <Tabel2B1Client initialRows={rows} tahunAkademikId={activeTa.id} tabelKode={def.kode} status={status} userRole={session.user.role as Role} />
      </ErrorBoundary>
      {history.length > 0 && (
