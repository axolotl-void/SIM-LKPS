...
const Tabel2B3Client = dynamic(() =>
  import("@/components/tables/tabel-2b3-client").then((m) => m.Tabel2B3Client)
);
...
export default async function Tabel2B3Page() {
  ...
  const [activeTa, def] = await Promise.all([
    await db.tahunAkademik.findFirst({ where: { isActive: true } }),
    await db.tabelDefinition.findUnique({ where: { kode: "2.B.3" } }),
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <ErrorBoundary>
        <Tabel2B3Client initialRows={lkpsTs?.rows || []} tahunAkademikId={activeTa.id} tabelKode={def.kode} status={status} userRole={session.user.role as Role} />
      </ErrorBoundary>
      {history.length > 0 && (
