...
const Tabel51Client = dynamic(() =>
  import("@/components/tables/tabel-51-client").then((m) => m.Tabel51Client)
);
...
export default async function Tabel51Page() {
  ...
  const def = await db.tabelDefinition.findUnique({ where: { kode: "5.1" } });
  if (!def) { return <div className="rounded-2xl bg-white p-6 shadow-soft text-center text-xs font-semibold text-slate-500">Definisi Tabel 5.1 tidak ditemukan.</div>; }
  ...
  const rows = rawRows.map((r) => {
    ...
    return {
      id: r.id, rowOrder: r.rowOrder,
      rowData: {
        tahun: String(d.tahun ?? "daftar"),
        jenisTataKelola: String(d.jenisTataKelola ?? ""),
        namaSistem: String(d.namaSistem ?? ""),
        akses: (d.akses === "Internet" ? "Internet" : "Lokal") as "Lokal" | "Internet",
        unitPengelola: String(d.unitPengelola ?? ""),
  ...
  return (
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-2xl bg-white p-4 shadow-soft-sm border border-slate-100/60">
        <div className="absolute right-0 top-0 bottom-0 w-1/3 bg-gradient-to-l from-slate-50/50 via-slate-50/20 to-transparent pointer-events-none rounded-r-3xl" />
        <div className="absolute right-12 top-1/2 -translate-y-1/2 hidden md:block pointer-events-none">
          <div className="flex h-20 w-16 rotate-12 items-center justify-center rounded-2xl bg-white shadow-soft-lg border border-slate-100/40 text-slate-500">
        ...
        <div className="relative z-10 flex flex-col gap-2.5 md:max-w-2xl">
          <div>
            <div className="flex items-center gap-3">
              <span className="text-3xs font-black uppercase tracking-wider text-slate-600 bg-slate-50/80 px-2.5 py-1 rounded-lg">Tabel {def.kode}</span>
            ...
            <h2 className="mt-1.5 text-base font-bold text-slate-800 tracking-tight">{def.nama}</h2>
            <p className="mt-1 text-xs font-semibold text-slate-500">Sistem informasi tata kelola di UPPS/PS</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-2 rounded-lg bg-slate-50/70 p-2">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-tr from-slate-500/10 to-purple-500/10 text-slate-600 shadow-soft-2xs">
