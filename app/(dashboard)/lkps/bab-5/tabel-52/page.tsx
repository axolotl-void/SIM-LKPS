...
const Tabel52Client = dynamic(() =>
  import("@/components/tables/tabel-52-client").then((m) => m.Tabel52Client)
);
...
export default async function Tabel52Page() {
  ...
  const [activeTa, def] = await Promise.all([
    ...
    await db.tabelDefinition.findUnique({ where: { kode: "5.2" } }),
  ]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex flex-col gap-2.5 md:max-w-2xl">
          <div>
            <div className="flex items-center gap-3">
              <span className="text-3xs font-black uppercase tracking-wider text-slate-600 bg-slate-50/80 px-2.5 py-1 rounded-lg">
                Tabel {def.kode}
              </span>
      ...
      <ErrorBoundary>
        <Tabel52Client initialRows={rows} tahunAkademikId={activeTa.id} tabelKode={def.kode} status={status} userRole={session.user.role as Role} />
      </ErrorBoundary>
